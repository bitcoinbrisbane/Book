...
The name comes from the analogy of gold miners prospecting for gold. As we saw in [the chapter on hash functions](../chapter-4-crypto-cryptography/3_hash-functions.md), a cryptographic hash output is effectively random for the user — there is no shortcut, no pattern, no way to predict in advance which input will produce a hash below the target.
