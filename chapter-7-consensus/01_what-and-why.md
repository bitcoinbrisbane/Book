...
When I worked at ConsenSys, my colleague Tom described at consensus mechanism this way.  Ill adapt to the theme of poker;

Lets say Alice and Bob are playing poker.  Each player witnesses the deal, the flop, the turn and the river.  Once we reach show down and the hole cards are revieled, consensus on the who won is reached.  Obviously, sometimes arguments can occur, but for this thought experiment, lets assume not.
...
The most common solution to this problem is based on commitment schemes, specifically Blum's protocol, which ensures fairness by using cryptography to prevent either party from cheating.
 ...
 Alice commits to a bit (0 or 1):


Alice picks a random bit aaa (either 0 or 1), which represents her initial choice for the coin flip.

Alice generates a cryptographic commitment to aaa using a commitment function C(a,r)C(a, r)C(a,r), where rrr is a random number (called a nonce) used to secure the commitment.

Alice sends the commitment C(a,r)C(a, r)C(a,r) to Bob. The commitment function hides aaa while ensuring that Alice can't change her choice later (she is "committed" to aaa).


Bob chooses a bit:


Bob picks a random bit bbb (either 0 or 1), which represents his choice for the coin flip.

Bob sends his choice bbb to Alice.

Alice reveals her bit:


After receiving Bob's choice, Alice reveals both her bit aaa and the random value rrr she used in the commitment.

Alice sends aaa and rrr to Bob.

Bob verifies the commitment:


Bob checks if the commitment C(a,r)C(a, r)C(a,r) matches the value Alice originally sent. If it does, this confirms that Alice did not change her bit after learning Bob's bit.

Determine the result:


The result of the coin flip is computed as a⊕ba \oplus ba⊕b (XOR of Alice’s bit and Bob’s bit). This gives a fair result, as each outcome (0 or 1) has an equal probability of occurring.

Now lets see this as an example and some Javascript code
...
Alice picks a=0a = 0a=0 (for heads) and generates a cryptographic commitment: C(a,r)C(a, r)C(a,r).
...
Bob picks b=1b = 1b=1 (for tails) and sends it to Alice.

Reveal Phase:


Alice reveals her choice a=0a = 0a=0 and the random nonce rrr used to create the commitment.
Bob verifies that the commitment matches Alice’s original message.

Outcome:

The result of the coin flip is 0⊕1=10 \oplus 1 = 10⊕1=1 (tails). The coin flip result is fair.
...
Although these are cryptography examples of message protocols, they're also protocols in wich the players, or nodes on the network can agree on the state of network.  Or reach consensus. 