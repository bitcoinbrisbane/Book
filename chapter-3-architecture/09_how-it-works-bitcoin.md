...
The next five pages walk through it layer-by-layer with code snippets, starting from [Toy Node — Overview](./11_toy-node-overview.md). The code itself lives in [`code/csharp_bitcoin_node/`](../code/csharp_bitcoin_node/) — `dotnet run` is enough to watch a node bootstrap itself.
